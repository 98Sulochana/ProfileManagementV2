...
begin
  ...
  begin
    ...
    begin
      ...
      TelValid := (Length(TelNumber) = 10) and (TelNumber[1] = '0') and
        (StrToIntDef(TelNumber, -1) >= 0);

      if not TelValid then
      begin
        ShowMessage('Please enter a 10-digit valid telephone number.');
      ...
      // Show success message when data added to database
      ShowMessage('Profile data added successfully!');
  ...
  begin
    repeat
      ...
      begin
        ...
        // Show success message when data edit
        ShowMessage('Profile data edit successfully!');
        // Refresh data
        actrefreshdata.Execute;
...
begin
  ...
  begin
    ...
    if TryStrToInt(SearchText, SearchAsInt) then
      FilterExpression := FilterExpression + ' OR (id = ' +
        inttostr(SearchAsInt) + ')';
